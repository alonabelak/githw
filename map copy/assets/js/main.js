...
let greenIcon = new myIcon({iconUrl: 'img/map-green.png'}),
    redIcon = new myIcon({iconUrl: 'img/map-red.png'});
...
function initMap(){
    ...
    for (let i = 0; i < PLACES.length; i++) {
        let marker = L.marker([PLACES[i].lat, PLACES[i].lng], { icon: new myIcon({ iconUrl: 'img/map-' + PLACES[i].type + '.png' }) }) .addTo(map);
        marker.bindPopup(`<ul>
